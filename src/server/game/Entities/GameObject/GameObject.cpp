...
void GameObject::Use(Unit* user)
{
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_BARBER_CHAIR:                  //32
        {
            ...
            if (player->GetRace() == RACE_GOBLIN || player->GetRace() == RACE_WORGEN || player->GetRace() == RACE_FOREST_TROLL || player->GetRace() == RACE_HIGHELF)
            {
                player->GetSession()->SendNotification("Custom races cannot use the barber shop. You have been flagged for recustomization at the character screen.");
