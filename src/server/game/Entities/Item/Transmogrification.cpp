...
// EMBER
    // Transmog Shield onto weapon
    if ((source->InventoryType == INVTYPE_SHIELD) && (target->InventoryType == INVTYPE_WEAPON || target->InventoryType == INVTYPE_WEAPONOFFHAND || target->InventoryType == INVTYPE_2HWEAPON))
        return true;
    // Transmog 1h Weapons onto shield
    if ((source->InventoryType == INVTYPE_WEAPON || source->InventoryType == INVTYPE_WEAPONOFFHAND || source->InventoryType == INVTYPE_2HWEAPON) && (target->InventoryType == INVTYPE_SHIELD))
        return true;
    // Transmog Holdables onto 1h weapons and shield
    if ((source->InventoryType == INVTYPE_HOLDABLE) && (target->InventoryType == INVTYPE_WEAPON || target->InventoryType == INVTYPE_WEAPONOFFHAND || target->InventoryType == INVTYPE_SHIELD || target->InventoryType == INVTYPE_2HWEAPON))
        return true;
